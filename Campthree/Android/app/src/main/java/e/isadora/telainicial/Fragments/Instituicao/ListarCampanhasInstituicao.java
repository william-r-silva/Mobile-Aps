...
public class ListarCampanhasInstituicao extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        service.buscarAutorizadas(AuxGlobal.instituicao.getId()).enqueue(new Callback<List<Campanha>>() {
            @Override
            public void onResponse(Call<List<Campanha>> call, Response<List<Campanha>> response) {
                if(response.isSuccessful()){
                    lista.addAll(response.body());
                    if(lista.size() != 0) {
                        AdapterInstituicaoCampanha adapterCampanha = new AdapterInstituicaoCampanha(getContext(), lista);
