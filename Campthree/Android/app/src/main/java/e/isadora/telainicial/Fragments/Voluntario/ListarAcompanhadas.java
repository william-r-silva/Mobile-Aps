...
public class ListarAcompanhadas extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        service.buscarParticipadas(AuxGlobal.user.getId()).enqueue(new Callback<List<Campanha>>() {
            @Override
            public void onResponse(Call<List<Campanha>> call, Response<List<Campanha>> response) {
                if(response.isSuccessful()) {
                     lista.addAll(response.body());
                     AdapterCampanha adapterCampanha = new AdapterCampanha(getContext(), lista, 1);
